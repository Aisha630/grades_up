...
class GradeChecker:
    ...
    def scrape_grades(self, page: Page) -> dict:
        ...
        page.wait_for_selector(
            'table#TERM_CLASSES\\$scroll\\$0 table.PSLEVEL1GRID', timeout=10000)
        rows = page.locator(
            'table#TERM_CLASSES\\$scroll\\$0 table.PSLEVEL1GRID tr')
        ...
        with sync_playwright() as p:
            ...
            while retries <= MAX_RETRIES:
                try:
                    ...
                    if success:
                        logger.info(
                            f"Check complete. Sleeping {check_interval} seconds...")
                    else:
                        logger.warning("Check failed, retrying sooner...")

                    time.sleep(
                        check_interval if success else DEFAULT_RETRY_DELAY)
...
def main():
    ...
    checker.run(check_interval=DEFAULT_CHECK_INTERVAL,
                headless=False)  # 10 minutes
